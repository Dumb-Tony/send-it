...
The source conversation expresses an eventual GitHub repo and shareable browser build. Current task explicitly requests local project/repository creation and beginning M1; this delivery establishes that locally. Remote creation/deployment is a subsequent concrete step, with account/repo visibility resolved from the user's instructions at that time. Static paths are relative for project-subpath hosting. No bundler is necessary: publish the tracked site files, excluding docs/tests/dev scripts as appropriate, via a reviewed Pages workflow. Test the actual public URL and console after deployment. Roll back to a known release commit if a build regresses. Never claim a deployment exists based on a workflow file alone.
